...
impl Ext4FileSystem {
    ...
    ///创建lost+found
    fn create_lost_found_dir<B: BlockDevice>(
        &mut self,
        block_dev: &mut BlockDev<B>,
    ) -> BlockDevResult<()> {
    ...
    fn load_group_descriptors<B: BlockDevice>(
        ...
        debug!(
            "Loading group descriptors: {} groups, desc_size = {} bytes",
            group_count, desc_size
        );
    ...
    pub fn alloc_block<B: BlockDevice>(
        ...
        debug!(
            "Allocated block: group={}, block_in_group={}, global_block={} [delayed write]",
    ...
    pub fn alloc_inode<B: BlockDevice>(
        ...
        // 7. 更新超级块
        self.superblock.s_free_inodes_count = self.superblock.s_free_inodes_count.saturating_sub(1);

        debug!(
            "Allocated inode: group={}, inode_in_group={}, global_inode={} [delayed write]",
...
pub fn compute_fs_layout(total_blocks: u64) -> FsLayoutInfo {
    ...
    // 块组数：向上取整
    let groups: u32 =
        ((total_blocks + blocks_per_group as u64 - 1) / blocks_per_group as u64) as u32;
...
/// 读取超级块 管字节序
fn read_superblock<B: BlockDevice>(block_dev: &mut BlockDev<B>) -> BlockDevResult<Ext4Superblock> {
    // 超级块总是从分区偏移 1024 字节开始，占用 1024 字节
